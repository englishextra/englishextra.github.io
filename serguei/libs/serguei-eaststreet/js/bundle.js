...
(function(root, document) {
	...
	var loadJS = function loadJS(_src, callback) {
...
(function(root, document) {
	...
	var run = function run() {
		...
		var loadExternalHTML = function loadExternalHTML(
			url,
			callback,
			onerror
		) {
			...
			x.onreadystatechange = function() {
				if (x.status === 404 || x.status === 0) {
					console.log(
						"Error XMLHttpRequest-ing file " + url,
						x.status
					);
					return (
						onerror && "function" === typeof onerror && onerror()
		...
		var initNotifier42WriteMe = function initNotifier42WriteMe() {
			...
			var msgText =
				"Напишите мне, отвечу очень скоро. Регистрироваться не нужно.";
		...
		var initSidepanel = function initSidepanel() {
			var btn =
			...
			var overlay =
				document[getElementsByClassName]("page-overlay")[0] || "";
			var panel =
				document[getElementsByClassName]("ui-sidepanel")[0] || "";
			...
			var arrange = function arrange() {
				var handleOtherUIElementAll = function handleOtherUIElementAll() {
				...
				var handleBtnSidepanel = function handleBtnSidepanel(ev) {
					ev.stopPropagation();
					ev.preventDefault();
				...
				var handleOverlaySidepanel = function handleOverlaySidepanel() {
				...
				var handleContainerSidepanel = function handleContainerSidepanel() {
					if (!page[classList].contains(isActiveSidepanelClass)) {
				...
				if (root.tocca) {
					if (
						...
						overlay[_addEventListener](
							"swipeleft",
							handleOverlaySidepanel
						);

						container[_addEventListener](
							"swiperight",
							handleContainerSidepanel
						);
					}
				}
		...
		var highlightSidepanelItem = function highlightSidepanelItem() {
			var panel =
				document[getElementsByClassName]("ui-sidepanel-list")[0] || "";
		...
		var initMenuMore = function initMenuMore() {
			var btn =
				document[getElementsByClassName]("btn-toggle-ui-menumore")[0] ||
				"";
			var page = document[getElementsByClassName]("page")[0] || "";
			var holder =
				document[getElementsByClassName]("ui-menumore")[0] || "";
			...
			var arrange = function arrange() {
				...
				var handleContainer = function handleContainer(ev) {
					ev.stopPropagation();
					ev.preventDefault();
		...
		var manageDataQrcodeImageAll = function manageDataQrcodeImageAll(
			...
			var img = ctx
				? ctx[getElementsByClassName](dataQrcodeImgClass) || ""
		...
		var includeHTMLintoTarget = function includeHTMLintoTarget(
			_this,
			u,
			t
		) {
			...
			var arrangeContainer = function arrangeContainer() {
				...
				var processResponse = function processResponse(t) {
					var cb = function cb() {
						hideBtn();
				...
				var hideAllOnError = function hideAllOnError() {
					hideBtn();
		...
		var manageDataTargetLinks = function manageDataTargetLinks(scope) {
			var ctx = scope && scope.nodeName ? scope : "";
		...
		var manageLocationQrCodeImage = function manageLocationQrCodeImage() {
			...
			var handleGenerateLocationQrCodeImgBtn = function handleGenerateLocationQrCodeImgBtn(
				ev
			) {
				ev.stopPropagation();
				ev.preventDefault();
			...
			var removePageIsActiveClass = function removePageIsActiveClass() {
				if (page[classList].contains(isActiveQRCodeClass)) {
					page[classList].remove(isActiveQRCodeClass);
				}
			};

			var handleGenerateLocationQrCodeImgHolder = function handleGenerateLocationQrCodeImgHolder() {
				removePageIsActiveClass();
				handleOtherUIElementAll();
			};

			var generateLocationQrCodeImg = function generateLocationQrCodeImg() {
				var locationHref = root.location.href || "";
			...
			var initScript = function initScript() {
				...
				btn[_addEventListener](
					"click",
					handleGenerateLocationQrCodeImgBtn
				);
				...
				holder[_addEventListener](
					"click",
					handleGenerateLocationQrCodeImgHolder
				);
			};
		...
		var loadRefreshDisqus = function loadRefreshDisqus() {
		...
		var manageDisqusButton = function manageDisqusButton() {
			var disqusThread = document[getElementById]("disqus_thread") || "";
			var btn = disqusThread
				? document[getElementsByClassName]("btn-show-disqus")[0] || ""
				: "";

			var handleManageDisqusButton = function handleManageDisqusButton(
				ev
			) {
				ev.stopPropagation();
				ev.preventDefault();
		...
		var initYandexMap = function initYandexMap(yandexMapId) {
			var yandexMap = document[getElementById](yandexMapId) || "";
			var btnShow = yandexMap
				? document[getElementById](yandexMap[dataset].btnShow) || ""
				: "";
			var btnDestroy = yandexMap
				? document[getElementById](yandexMap[dataset].btnDestroy) || ""
				: "";
			var yandexMapCenter = yandexMap
				? yandexMap[dataset].center || ""
				: "";
			var yandexMapZoom = yandexMap ? yandexMap[dataset].zoom || "" : "";
			var isActiveClass = "is-active";

			var handleYandexMapBtnDestroy = function handleYandexMapBtnDestroy(
				ev
			) {
				ev.stopPropagation();
				ev.preventDefault();

				btnDestroy[_removeEventListener](
					"click",
					handleYandexMapBtnDestroy
				);
			...
			var initScript = function initScript() {
				var initMyMap = function initMyMap() {
					if (mymap) {
						mymap.destroy();
					}
			...
			if (yandexMap && yandexMapCenter && yandexMapZoom && btnShow) {
				if ("undefined" !== typeof getHTTP && getHTTP()) {
					...
					var jsUrl =
						forcedHTTP + "://api-maps.yandex.ru/2.1/?lang=ru_RU";
					...
					var msgText = document[
						...
						"<p>Карты доступны только в веб версии этой страницы.</p>"
					);
		...
		var manageYandexMapButton = function manageYandexMapButton(
			yandexMapId
		) {
			var yandexMap = document[getElementById](yandexMapId) || "";
			var btnShow = yandexMap
			...
			var handleBtnShow = function handleBtnShow(ev) {
				ev.stopPropagation();
				ev.preventDefault();
		...
		var initRoutie = function initRoutie() {
			...
			var loadVirtualPage = function loadVirtualPage(c, h, f) {
				if (c && h) {
					LoadingSpinner.show();
			...
			var reinitVirtualPage = function reinitVirtualPage(titleString) {
			...
			var loadNotFoundPage = function loadNotFoundPage(containerClass) {
				var container = document[getElementById](containerClass) || "";
				var msgText = document[createRange]().createContextualFragment(
					'<div class="padded-content"><div class="col"><div class="row"><div class="column small-12 medium-12 large-12"><p>Нет такой страницы. <a href="#/home">Исправить?</a></p></div></div></div></div>'
				);
			...
			if (appContent) {
				routie({
					"": function _() {
						loadVirtualPage(
							appContentId,
							"./includes/serguei-eaststreet/home.html",
							function() {
					...
					"/schedule": function schedule() {
						if ("undefined" !== typeof getHTTP && getHTTP()) {
							loadVirtualPage(
								appContentId,
								"./includes/serguei-eaststreet/schedule.html",
								function() {
					...
					"/map": function map() {
						if ("undefined" !== typeof getHTTP && getHTTP()) {
							loadVirtualPage(
								appContentId,
								"./includes/serguei-eaststreet/map.html",
								function() {
					...
					"/level_test": function level_test() {
						loadVirtualPage(
							appContentId,
							"./includes/serguei-eaststreet/level_test.html",
							function() {
					...
					"/common_mistakes": function common_mistakes() {
						loadVirtualPage(
							appContentId,
							"./includes/serguei-eaststreet/common_mistakes.html",
							function() {
								reinitVirtualPage("Распространенные ошибки");
							}
						);
					},
					"/demo_ege": function demo_ege() {
						loadVirtualPage(
							appContentId,
							"./includes/serguei-eaststreet/demo_ege.html",
							function() {
								reinitVirtualPage(
									"Демо-вариант ЕГЭ-11 АЯ (ПЧ)"
								);
							}
						);
					},
					"/demo_ege_speaking": function demo_ege_speaking() {
						loadVirtualPage(
							appContentId,
							"./includes/serguei-eaststreet/demo_ege_speaking.html",
							function() {
								reinitVirtualPage(
									"Демо-вариант ЕГЭ-11 АЯ (УЧ)"
								);
							}
						);
					},
					"/previous_ege_analysis": function previous_ege_analysis() {
						loadVirtualPage(
							appContentId,
							"./includes/serguei-eaststreet/previous_ege_analysis.html",
							function() {
