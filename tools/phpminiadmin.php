...
*/
 ...
 $DB=array(
 'user'=>MYSQL_DB_USER,#required
 'pwd'=>MYSQL_DB_PASS, #required
 'db'=>MYSQL_DB_NAME,	#default DB, optional
 'host'=>MYSQL_DB_HOST,#optional
 'port'=>"",#optional
...
&nbsp;&#183;<a href='?db=$dbn&q=show+processlist'>Show Processlist</a><br />";
...
SQL-query (or many queries separated by ";"):<br /><textarea name="q" cols="70" rows="10" style="width:98%"><?php echo $SQLq?></textarea><br /><input type=submit name="GoSQL" value="Go" onClick="return chksql()" style="width:100px">&nbsp;&nbsp;
...
Records: <b><?php echo $reccount?></b> in <b><?php echo $time_all?></b> sec<br /><b><?php echo $out_message?></b>
</div>
<?php
...
<div class="frm">
User name: <input type="text" name="v[user]" value="<?php echo $DB['user']?>"><br />Password: <input type="password" name="v[pwd]" value=""><br />MySQL host: <input type="text" name="v[host]" value="<?php echo $DB['host']?>"> port: <input type="text" name="v[port]" value="<?php echo $DB['port']?>" size="4"><br />DB name: <input type="text" name="v[db]" value="<?php echo $DB['db']?>"><br />Charset: <select name="v[chset]"><option value="">- default -</option><?php echo chset_select($DB['chset'])?></select><br /><input type="checkbox" name="rmb" value="1" checked> Remember in cookies for 30 days
...
<div class="frm">
<input type="checkbox" name="s" value="1" checked> Structure<br /><input type="checkbox" name="d" value="1" checked> Data<br /><br /><input type="radio" name="et" value="" checked> .sql<br /><?php if ($t && !strpos($t,',')){?>
...
<?php }?><br /><br /><input type="hidden" name="doex" value="1">
...
<h3>Import DB</h3>
<div class="frm">
.sql file: <input type="file" name="file1" value="" size=40><br /><input type="hidden" name="doim" value="1">
...
<div class="frm">
.csv file (Excel style): <input type="file" name="file2" value="" size=40><br /><input type="checkbox" name="r1" value="1" checked> first row contain field names<br /><small>(note: for success, field names should be exactly the same as in DB)</small><br />Character set of the file: <select name="chset"><?php echo chset_select('utf8')?></select><br /><br />Import into:<br /><input type="radio" name="tt" value="1" checked="checked"> existing table:
 <select name="t">
 <option value=''>- select -</option>
 <?php echo sel(db_array('show tables',NULL,0,1), 0, '');?>
</select>
<div style="margin-left:20px">
 <input type="checkbox" name="ttr" value="1"> replace existing DB data<br /><input type="checkbox" name="tti" value="1"> ignore duplicate rows
</div>
<input type="radio" name="tt" value="2"> create new table with name <input type="text" name="tn" value="" size="20"><br /><br /><input type="hidden" name="doimcsv" value="1">
<input type="submit" value=" Upload and Import " onclick="return ays()"><input type="button" value=" Cancel " onclick="window.location='<?php echo $self?>'">
</div>
-->
</center>
