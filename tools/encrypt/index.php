...
"\u2026"
	...
	public function fix_filename_str($s) {
		$s = $this->safe_str($s);
		//remove tags
	 ...
	 * replace with _: , . : [ ] { } -
	 */
	public function unix_filename($s) {
		$s = str_replace(array("\n", "\r", "\t", "\v", "\0", "\x0B"), '', preg_replace("/[^\x20-\xFF]/", "", trim(@strval($s))));
		$s = trim(preg_replace("/[\ ]+/", " ", $s));
		$a = null;
		$a = array('¡', '¢', '£', '¤', '¥', '¦', '§', '¨', '©', 'ª', '«', '¬', '­', '®', '¯', '°', '±', '²', '³', '´', 'µ', '¶', '·', '¸', '¹', 'º', '»', '¼', '½', '¾', '¿', 'À', 'Á', 'Â', 'Ã', 'Ä', 'Å', 'Æ', 'Ç', 'È', 'É', 'Ê', 'Ë', 'Ì', 'Í', 'Î', 'Ï', 'Ð', 'Ñ', 'Ò', 'Ó', 'Ô', 'Õ', 'Ö', '×', 'Ø', 'Ù', 'Ú', 'Û', 'Ü', 'Ý', 'Þ', 'ß', 'à', 'á', 'â', 'ã', 'ä', 'å', 'æ', 'ç', 'è', 'é', 'ê', 'ë', 'ì', 'í', 'î', 'ï', 'ð', 'ñ', 'ò', 'ó', 'ô', 'õ', 'ö', '÷', 'ø', 'ù', 'ú', 'û', 'ü', 'ý', 'þ', 'ÿ', 'Œ', 'œ', 'Š', 'š', 'Ÿ', 'ƒ', 'ˆ', '˜', 'Α', 'Β', 'Γ', 'Δ', 'Ε', 'Ζ', 'Η', 'Θ', 'Ι', 'Κ', 'Λ', 'Μ', 'Ν', 'Ξ', 'Ο', 'Π', 'Ρ', 'Σ', 'Τ', 'Υ', 'Φ', 'Χ', 'Ψ', 'Ω', 'α', 'β', 'γ', 'δ', 'ε', 'ζ', 'η', 'θ', 'ι', 'κ', 'λ', 'μ', 'ν', 'ξ', 'ο', 'π', 'ρ', 'ς', 'σ', 'τ', 'υ', 'φ', 'χ', 'ψ', 'ω', 'ϑ', 'ϒ', 'ϖ', ' ', ' ', ' ', '‌', '‍', '‎', '‏', '–', '—', '‘', '’', '‚', '“', '”', '„', '†', '‡', '•', '…', '‰', '′', '″', '‹', '›', '‾', '⁄', '€', 'ℑ', '℘', 'ℜ', '™', 'ℵ', '←', '↑', '→', '↓', '↔', '↵', '⇐', '⇑', '⇒', '⇓', '⇔', '∀', '∂', '∃', '∅', '∇', '∈', '∉', '∋', '∏', '∑', '−', '∗', '√', '∝', '∞', '∠', '∧', '∨', '∩', '∪', '∫', '∴', '∼', '≅', '≈', '≠', '≡', '≤', '≥', '⊂', '⊃', '⊄', '⊆', '⊇', '⊕', '⊗', '⊥', '⋅', '⌈', '⌉', '⌊', '⌋', '◊', '♠', '♣', '♥', '♦', '⟨', '⟩', '!', '"', '$', '%', '(', ')', '*', '+', ',', '-', '.', '/', ':', '<', '=', '>', '?', '@', '[', '\'', '\\', ']', '^', '_', '`', '{', '|', '}', '~', '✁', '✂', '✃', '✄', '✆', '✇', '✈', '✉', '✌', '✍', '✎', '✏', '✐', '✑', '✒', '✓', '✔', '✕', '✖', '✗', '✘', '✙', '✚', '✛', '✜', '✝', '✞', '✟', '✠', '✡', '✢', '✣', '✤', '✥', '✦', '✧', '✩', '✪', '✫', '✬', '✭', '✮', '✯', '✰', '✱', '✲', '✳', '✴', '✵', '✶', '✷', '✸', '✹', '✺', '✻', '✼', '✽', '✾', '✿', '❀', '❁', '❂', '❃', '❄', '❅', '❆', '❇', '❈', '❉', '❊', '❋', '❍', '❏', '❐', '❑', '❒', '❖', '❘', '❙', '❚', '❛', '❜', '❝', '❞', '❡', '❢', '❣', '❤', '❥', '❦', '❧', '❶', '❷', '❸', '❹', '❺', '❻', '❼', '❽', '❾', '❿', '➀', '➁', '➂', '➃', '➄', '➅', '➆', '➇', '➈', '➉', '➊', '➋', '➌', '➍', '➎', '➏', '➐', '➑', '➒', '➓', '➔', '➘', '➙', '➚', '➛', '➜', '➝', '➞', '➟', '➠', '➡', '➢', '➣', '➤', '➥', '➦', '➧', '➨', '➩', '➪', '➫', '➬', '➭', '➮', '➯', '➱', '➲', '➳', '➴', '➵', '➶', '➷', '➸', '➹', '➺', '➻', '➼', '➽', '➾');
		foreach($a as $v) {
			$s = str_replace($v, "_", $s);
		}
	...
	public function fix_typo($s) {
		...
		if (preg_match("/[a-z]+/i", $s)) {
			$s = str_replace($us_locale, $ru_locale, $s);
		} else {
			$s = str_replace($ru_locale, $us_locale, $s);
		}
		return htmlspecialchars($s);
	}
}
...
if (!empty($source)) {
	...
	$p5 = $source;
	if (!empty($safe_numbers)) {$p5 = htmlspecialchars($Encrypt->conv_symbs_to_ents($Entities->text_digits_to_dec_ents($p5)));}
	$p6 = $source;
	if (!empty($safe_latin_chars)) {$p6 = htmlspecialchars($Encrypt->conv_symbs_to_ents($Entities->latin_text_chars_to_dec_ents($p6)));}
}
		...
		<div class="container">
			...
			<fieldset>
				<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="application/x-www-form-urlencoded">
					<p>
						<input name="event" type="hidden" value="md5" />
						<label for="source">text:</label>
						<textarea name="source" id="source" cols="30" rows="3"><?php
					...
					<p>
						...
						<label for="safe_numbers">safe_numbers</label>
						<input name="safe_numbers" id="safe_numbers" type="checkbox" />
						<label for="safe_latin_chars">safe_latin_chars</label>
						<input name="safe_latin_chars" id="safe_latin_chars" type="checkbox" />
						<input name="f_reset" id="f_reset" onclick="document.getElementById('source').innerHTML=''" value="Reset" class="button" type="reset" />
						<input name="f_submit" id="f_submit" value="Submit" class="button" type="submit" />
					</p>
				</form>
				<form id="results_form" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="application/x-www-form-urlencoded">
					<p><label>md5:</label><input type="text" value="<?php if (!empty($source)) {echo md5($source);} ?>" maxlength="255" size="80" /></p>
					<p><label>crypt:</label><input type="text" value="<?php if (!empty($source)) {echo crypt($source);} ?>" maxlength="255" size="80" /></p>
					<p><label>crc32:</label><input type="text" value="<?php if (!empty($source)) {echo crc32($source);} ?>" maxlength="255" size="80" /></p>
					<p><label>sha1:</label><input type="text" value="<?php if (!empty($source)) {echo sha1($source);} ?>" maxlength="255" size="80" /></p>
					<p><label>base64:</label><input type="text" value="<?php if (!empty($source)) {echo base64_encode($source);} ?>" maxlength="255" size="80" /></p>
					<p><label>urlencode:</label><input type="text" value="<?php if (!empty($source)) {echo htmlspecialchars(urlencode($source));} ?>" maxlength="255" size="80" /></p>
					<p><label>urldecode:</label><input type="text" value="<?php if (!empty($source)) {echo htmlspecialchars(urldecode($source));} ?>" maxlength="255" size="80" /></p>
					<p><label>typo:</label><input type="text" value="<?php
						if (!empty($p4)) {echo $p4;}
						?>" maxlength="255" size="80" /></p>
					<p><label>unix_filename:</label><input type="text" value="<?php
						if (!empty($p1)) {echo $p1;}
						?>" maxlength="255" size="80" /></p>
					<p><label>htmlentities:</label><textarea name="htmlentities" id="htmlentities" cols="30" rows="3"><?php
						if (!empty($p)) {echo $p;}
						?></textarea></p>
					<p><label>safe_numbers:</label><input type="text" value="<?php
						if (!empty($p5)) {echo $p5;}
						?>" maxlength="255" size="80" /></p>
					<p><label>safe_latin_chars:</label><input type="text" value="<?php
						if (!empty($p6)) {echo $p6;}
						?>" maxlength="255" size="80" /></p>
					<p><label>java_escape:</label><textarea cols="30" rows="3"><?php
						if (!empty($p3)) {echo $p3;}
						?></textarea></p>
				</form>
			</fieldset>
