...
(function(root, document) {
	...
	var Carousel = function Carousel(setting) {
		var _this = this;
		...
		this.prev_slide = function() {
			...
			privates.sel.wrap.style.transform =
		...
		privates.sel = {
			...
			children: document[getElementsByClassName](privates.setting.wrap)[0]
				.children,
		...
		privates.opt = {
			position: 0,
			max_position: document[getElementsByClassName](
				privates.setting.wrap
			)[0].children[_length]
		};
...
(function(root, document) {
	...
	var run = function run() {
		...
		var alignToMasterBottomLeft = function alignToMasterBottomLeft(
			masterId,
			servantId,
			sameWidth
		) {
		...
		var handleOtherDropdownLists = function handleOtherDropdownLists(
			_self
		) {
		...
		var manageOtherDropdownListAll = function manageOtherDropdownListAll() {
		...
		var manageChaptersSelect = function manageChaptersSelect() {
			...
			var holderChaptersSelect =
				document[getElementsByClassName]("holder-chapters-select")[0] ||
				"";
			var uiPanelContentsSelect =
				document[getElementsByClassName](
					"ui-panel-contents-select"
				)[0] || "";
      	...
      	for (var i = 0, l = chaptersSelectOptions[_length]; i < l; i += 1) {
			...
			var rerenderChaptersList = function rerenderChaptersList() {
				var handleChaptersListItem = function handleChaptersListItem(
					listObj,
					hashString
				) {
					var uiPanelContentsSelectHeight = uiPanelContentsSelect
						? uiPanelContentsSelect[classList].contains(
								isFixedClass
						  )
							? uiPanelContentsSelect.offsetHeight
							: uiPanelContentsSelect.offsetHeight * 2
						: 0;
				...
				var chaptersListItems = chaptersSelect
					? chaptersSelect[getElementsByTagName]("option") || ""
					: "";
				var chaptersListButtonDefaultText = "";
				var df = document[createDocumentFragment]();

				var generateChaptersListItems = function generateChaptersListItems(
					_this,
					i
				) {
					if (0 === i) {
						chaptersListButtonDefaultText =
							_this.firstChild.textContent;
					}

					var chaptersListItem = document[createElement]("li");
					var chaptersListItemText =
						_this.firstChild.textContent || "";
					var chaptersListItemValue = _this.value;
					var chaptersListItemTextTruncated = truncString(
						"" + chaptersListItemText,
						28
					);
					chaptersListItem[appendChild](
						document[createTextNode](chaptersListItemTextTruncated)
					);
					chaptersListItem.title = chaptersListItemText;

					chaptersListItem[_addEventListener](
						"click",
						handleChaptersListItem.bind(
							null,
							chaptersList,
							chaptersListItemValue
						)
					);
				...
				holderChaptersSelect.replaceChild(
					chaptersList,
					chaptersSelect[parentNode][parentNode]
				);
				var chaptersListButton = document[createElement]("a");
				chaptersListButton[appendChild](
					document[createTextNode](chaptersListButtonDefaultText)
				);
				chaptersList[parentNode].insertBefore(
					chaptersListButton,
					chaptersList
				);
				/* jshint -W107 */
				...
				var insertChevronDownSmallSvg = function insertChevronDownSmallSvg(
					targetObj
				) {
				...
				var handleChaptersListItemsButton = function handleChaptersListItemsButton(
					ev
				) {
					ev.stopPropagation();
					ev.preventDefault();
				...
				chaptersListButton[_addEventListener](
					"click",
					handleChaptersListItemsButton
				);
			};
		...
		var initMasonry = function initMasonry(scope) {
			var ctx = scope && scope.nodeName ? scope : "";
			...
			var grid = ctx
				? ctx[getElementsByClassName](gridClass)[0] || ""
				: document[getElementsByClassName](gridClass)[0] || "";
			var gridItem = ctx
				? ctx[getElementsByClassName](gridItemClass)[0] || ""
		...
		var manageDisqusButton = function manageDisqusButton(scope) {
			var ctx = scope && scope.nodeName ? scope : "";
			var btnClass = "btn-show-disqus";
			var btn = ctx
				? ctx[getElementsByClassName](btnClass)[0] || ""
			...
			var hideDisqusButton = function hideDisqusButton() {
		...
		var notiBar = function notiBar(opt) {
			var notibarClass = "notibar";
			var notibarContainer =
				document[getElementsByClassName](notibarClass)[0] || "";
			...
			var insertCancelSvg = function insertCancelSvg(targetObj) {
					var svg = document[createElementNS](
						"http://www.w3.org/2000/svg",
						"svg"
					);
		...
		var initNotibarMsg = function initNotibarMsg() {
			var uiPanelContentsSelect =
				document[getElementsByClassName](
					"ui-panel-contents-select"
				)[0] || "";
			...
			var cookieDatum =
				"Выбрать статью можно щелкнув по самофиксирующейся планке с заголовком текущей страницы.";
			...
			var arrange = function arrange() {
				var timer = setTimeout(function() {
					...
					var handleMsgObj = function handleMsgObj(ev) {
						...
						var uiPanelContentsSelectPos = uiPanelContentsSelect
							? findPos(uiPanelContentsSelect).top
							: 0;
						var uiPanelContentsSelectHeight = uiPanelContentsSelect
							? uiPanelContentsSelect[classList].contains(
									isFixedClass
		...
		var initKamilAutocomplete = function initKamilAutocomplete(jsonObj) {
			var searchForm =
			...
			var initScript = function initScript() {
            	...
            	limitKamilOutput(e, i);
					...
					var logic = function logic() {
						while (itemsLength < 1) {
							...
							appendFragment(
								document[createTextNode]("" + textInputValue),
								typoListItem
							);
				...
				var handleTypoListItem = function handleTypoListItem(ev) {
					ev.stopPropagation();
					ev.preventDefault();
					/*!
		...
		var renderNavigation = function renderNavigation() {
			...
			var popularTemplate =
				document[getElementById](popularTemplateId) || "";
			...
			var carouselTemplate =
				document[getElementById](carouselTemplateId) || "";
			var carouselRenderId = "render_b_carousel";
			var carouselRender =
				document[getElementById](carouselRenderId) || "";
			var carouselRenderParent = carouselRender[parentNode] || "";
			var showRenderNavbarPopularId = "show_render_navbar_popular";
			var showRenderNavbarPopular =
				document[getElementById](showRenderNavbarPopularId) || "";
			var renderNavbarPopularId = "render_navbar_popular";
			var renderNavbarPopular =
				document[getElementById](renderNavbarPopularId) || "";
			var showRenderNavbarMoreId = "show_render_navbar_more";
			var showRenderNavbarMore =
				document[getElementById](showRenderNavbarMoreId) || "";
			var renderNavbarMoreId = "render_navbar_more";
			var renderNavbarMore =
				document[getElementById](renderNavbarMoreId) || "";
			var navigationJsonUrl =
				"./libs/pwa-englishextra/json/navigation.json";

			var processNavigationJsonResponse = function processNavigationJsonResponse(
				navigationJsonResponse
			) {
				try {
					...
					if (!navigationJsonObj.navbar_popular) {
						throw new Error(
							"incomplete JSON data: no navbar_popular"
						);
						...
						if (!navigationJsonObj.b_carousel) {
							throw new Error(
								"incomplete JSON data: no b_carousel"
							);
						}
					}
					...
					console.log(
						"cannot init processNavigationJsonResponse",
						err
					);
					return;
				}

				var handleListItemAll = function handleListItemAll(e) {
				...
				var handleShowRenderNavbarPopularButton = function handleShowRenderNavbarPopularButton(
					ev
				) {
					ev.stopPropagation();
					ev.preventDefault();
				...
				var handleShowRenderNavbarMoreButton = function handleShowRenderNavbarMoreButton(
					ev
				) {
					ev.stopPropagation();
					ev.preventDefault();
				...
				var alignNavbarListAll = function alignNavbarListAll() {
					alignToMasterBottomLeft(
						showRenderNavbarPopularId,
						renderNavbarPopularId
					);
					alignToMasterBottomLeft(
						showRenderNavbarMoreId,
						renderNavbarMoreId
					);
				};

				var handleShowNavbarListsWindow = function handleShowNavbarListsWindow() {
							...
							if (moreTemplate && moreRender) {
								insertFromTemplate(
									...
									function() {
										...
										showRenderNavbarPopular[
											_addEventListener
										](
											"click",
											handleShowRenderNavbarPopularButton
										);

										showRenderNavbarMore[_addEventListener](
											"click",
											handleShowRenderNavbarMoreButton
										);

										root[_addEventListener](
											"resize",
											handleShowNavbarListsWindow
										);
		...
		var fixUiPanelContentsSelect = function fixUiPanelContentsSelect() {
			var uiPanelNavigation =
				document[getElementsByClassName]("ui-panel-navigation")[0] ||
				"";
			var holderHero =
				document[getElementsByClassName]("holder-hero")[0] || "";
			var uiPanelContentsSelect =
				document[getElementsByClassName](
					"ui-panel-contents-select"
				)[0] || "";
			var criticalHeight =
				(uiPanelNavigation ? uiPanelNavigation.offsetHeight : 0) +
				(holderHero ? holderHero.offsetHeight : 0);

			var handleUiPanelContentsSelect = function handleUiPanelContentsSelect() {
				var logic = function logic() {
					if (
						(docBody.scrollTop || docElem.scrollTop || 0) >
						criticalHeight
					) {
			...
			if (uiPanelContentsSelect) {
				root[_addEventListener]("scroll", handleUiPanelContentsSelect, {
					passive: true
				});
			}
		};
		...
		var handleOtherSocialButtons = function handleOtherSocialButtons(
			_self
		) {
			var _this = _self || this;
		...
		var manageOtherSocialButtonAll = function manageOtherSocialButtonAll() {
			var container = document[getElementById]("container") || "";
		...
		var manageLocationQrCodeImage = function manageLocationQrCodeImage() {
			var btn =
				document[getElementsByClassName](
					"btn-toggle-holder-location-qrcode"
				)[0] || "";
			var holder =
		...
		var manageDebugGridButton = function manageDebugGridButton() {
			var container = document[getElementById]("container") || "";
			var page = document[getElementById]("page") || "";
			var btn =
				document[getElementsByClassName]("btn-toggle-col-debug")[0] ||
				"";
			var debugClass = "debug";
			var cookieKey = "_manageDebugGridButton_";
			var cookieDatum = "ok";

			var handleDebugGridButton = function handleDebugGridButton(ev) {
				ev.stopPropagation();
				ev.preventDefault();
				container[classList].toggle(debugClass);

				var showDebugGridMessage = function showDebugGridMessage() {
					var col = document[getElementsByClassName]("col")[0] || "";
					var elements = [docBody, page, container, col];
					var debugMessage = [];

					var renderElementsInfo = function renderElementsInfo(e) {
						if (e) {
							debugMessage.push(
								e.className
									? "." + e.className
					...
					debugMessage = debugMessage.slice(
						0,
						debugMessage.lastIndexOf(" > ")
					);
					notiBar({
				...
				var handleDebugGridContainer = function handleDebugGridContainer() {
					if (container) {
						container[classList].remove(debugClass);

						container[_removeEventListener](
							"click",
							handleDebugGridContainer
						);
					}
				};
			...
			if (page && container && btn) {
				...
				if (
					locationHref &&
					parseLink(locationHref).hasHTTP &&
					/^(localhost|127.0.0.1)/.test(
						parseLink(locationHref).hostname
					)
				) {
		...
		var initRouting = function initRouting() {
      ...
      var contentsSelectRender = document[getElementById]("render_contents_select") || ""; */
			...
			var contentsSelect =
				document[getElementsByClassName]("contents-select")[0] || "";
			var holderContentsSelect =
				document[getElementsByClassName]("holder-contents-select")[0] ||
				"";
			...
			var contentsListButtonDefaultText = contentsSelect
				? contentsSelect.options[0].firstChild.textContent || ""
				: "";
			var handleRoutesWindowIsBindedClass =
				"handle-routes-window--is-binded";

			var processRoutesJsonResponse = function processRoutesJsonResponse(
				routesJsonResponse
			) {
				var routesJsonObj;
				...
				try {
					...
					if (!routesJsonObj.hashes) {
						...
						if (!routesJsonObj.notfound) {
							throw new Error(
								"incomplete JSON data: no notfound"
							);
						}
					}
				} catch (err) {
          ...
          if (h1) {
          	...
          	scroll2Top(0, 20000);
					...
					if (contentsSelect) {
						...
						for (
							var i = 0, l = contentsSelect.options[_length];
							i < l;
							i += 1
						) {
							if (
								locationHash === contentsSelect.options[i].value
							) {
								optionMatched = true;
            	...
            	if (contentsSelect.options.hasOwnProperty(key)) {
					...
					if (contentsList) {
						var contentsListButton = holderContentsSelect
							? holderContentsSelect[getElementsByTagName](
									"a"
							  )[0] || ""
							: "";
						...
						if (contentsListButton) {
							...
							var contentsListItems = contentsList
								? contentsList[getElementsByTagName]("li") || ""
								: "";

							for (
								var j = 0, m = contentsListItems[_length];
								j < m;
								j += 1
							) {
								if (
									locationHash ===
									contentsListItems[j][dataset][href]
								) {
									itemMatched = true;
									contentsListButton.replaceChild(
										document[createTextNode](
											contentsListItems[j].firstChild
												.textContent
										),
										contentsListButton.firstChild
									);
									break;
								}
							}
              	...
              	if (contentsListItems.hasOwnProperty(key2)) {
					...
					if (asideObj) {
						var asideTemplate =
							document[getElementById](asideTemplateId) || "";
						var asideRender =
							document[getElementById](asideRenderId) || "";
					...
					if (nextHrefString) {
						var nextHrefTemplate =
							document[getElementById](nextHrefTemplateId) || "";
						var nextHrefRender =
							document[getElementById](nextHrefRenderId) || "";
					...
					var commentsTemplate =
						document[getElementById](commentsTemplateId) || "";
					var commentsRender =
						document[getElementById](commentsRenderId) || "";
					var commentsRenderParent = commentsRender[parentNode] || "";

					if (commentsTemplate && commentsRender) {
						if (!renderComments) {
							renderComments = renderTemplate(
								{},
								commentsTemplateId,
								commentsRenderId
							);
					...
					var masonryGrid =
						document[getElementsByClassName](masonryGridClass)[0] ||
						"";
					var masonryGridParent = masonryGrid[parentNode] || "";

					if (masonryGrid && masonryGridParent) {
						var contentsGridTemplate =
							document[getElementById](contentsGridTemplateId) ||
							"";
						var contentsGridRender =
							document[getElementById](contentsGridRenderId) ||
							"";
						var contentsGridRenderParent =
							contentsGridRender[parentNode] || "";

						if (contentsGridTemplate && contentsGridRender) {
							if (!renderMasonry) {
								if (routesObj) {
									renderMasonry = renderTemplate(
										routesObj,
										contentsGridTemplateId,
										contentsGridRenderId
									);
								}
							}
				...
				var handleRoutesWindow = function handleRoutesWindow() {
					...
					if (locationHash) {
						...
						for (
							var i = 0, l = routesJsonObj.hashes[_length];
							i < l;
							i += 1
						) {
							if (
								locationHash === routesJsonObj.hashes[i][href]
							) {
								isFound = true;
								LoadingSpinner.show();
								insertExternalHTML(
									appContentId,
									routesJsonObj.hashes[i].url,
									triggerOnContentInserted.bind(
										null,
										routesJsonObj.hashes[i][title],
										routesJsonObj.hashes[i].next_href,
										routesJsonObj.hashes[i].aside,
										routesJsonObj
									)
								);
								break;
        	...
        	contentsSelect[_addEventListener]("change", handleContentsSelect);
				...
				var rerenderContentsList = function rerenderContentsList() {
					var handleContentsListItem = function handleContentsListItem(
						listObj,
						hashString
					) {
						if (hashString) {
							var targetObj = isValidId(hashString, true)
								? document[getElementById](
										hashString.replace(/^#/, "")
								  ) || ""
								: "";
					...
					var contentsListButtonText =
						contentsSelect.options[0].textContent || "";
					var df = document[createDocumentFragment]();

					var generateContentsListItems = function generateContentsListItems(
						e
					) {
						if (e[title]) {
							var contentsListItem = document[createElement](
								"li"
							);
							var contentsListItemHref = e[href];
							var contentsListItemText = e[title];
							contentsListItem[title] = contentsListItemText;
							contentsListItem[dataset][
								href
							] = contentsListItemHref;
							var contentsListItemTextTruncated = truncString(
								"" + contentsListItemText,
								44
							);
							contentsListItem[appendChild](
								document[createTextNode](
									contentsListItemTextTruncated
								)
							);

							contentsListItem[_addEventListener](
								"click",
								handleContentsListItem.bind(
									null,
									contentsList,
									contentsListItemHref
								)
					...
					for (
						var j = 0, m = routesJsonObj.hashes[_length];
						j < m;
						j += 1
					) {
					...
					holderContentsSelect.replaceChild(
						contentsList,
						contentsSelect[parentNode][parentNode]
					);
					var contentsListButton = document[createElement]("a");
					contentsListButton[appendChild](
						document[createTextNode](contentsListButtonText)
					);
					contentsList[parentNode].insertBefore(
						contentsListButton,
						contentsList
					);
					/* jshint -W107 */
					...
					var handleContentsListItemsButton = function handleContentsListItemsButton(
						ev
					) {
						ev.stopPropagation();
						ev.preventDefault();
					...
					contentsListButton[_addEventListener](
						"click",
						handleContentsListItemsButton
					);
				};
