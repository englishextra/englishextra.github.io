...
(function(root, document) {
	...
	var run = function run() {
		...
		var setStyleOpacity = function setStyleOpacity(a, n) {
			n = n || 1;
		...
		var initSuperBox = function initSuperBox() {
			...
			var handleItem = function handleItem(_this) {
				var sDesc = _this
					? _this[getElementsByClassName](s5)[0] || ""
					: "";
				...
				var link = sCurDesc
					? sCurDesc[getElementsByTagName]("a") || ""
					: "";

				if (link) {
					var createCounterImg = function createCounterImg() {
						var _this = this;

						var rfrr = encodeURIComponent(root.location.href || ""),
							ttl = encodeURIComponent(
								document[title] || ""
							).replace("\x27", "&#39;"),
							hrefString = _this[getAttribute]("href") || "",
							dmn = hrefString
								? encodeURIComponent(hrefString)
								: "",
							counterHost = /^(localhost|127.0.0.1)/.test(
								root.location.host
							)
								? "http://localhost/externalcounters/"
								: "";
						...
						if (counterHost) {
							...
							counterElement[setAttribute](
								"style",
								"position:absolute;left:-9999px;width:1px;height:1px;border:0;background:transparent url(" +
									counterHost +
									"?dmn=" +
									dmn +
									"&rfrr=" +
									...
									"&ttl=" +
									ttl +
									"&encoding=utf-8) top left no-repeat;"
							);
							appendFragment(counterElement, docBody);
						}
					};

					var trackClicks = function trackClicks(e) {
						var hrefString = e[getAttribute]("href") || "",
							handleSuperboxExternalLink = function handleSuperboxExternalLink(
								ev
							) {
								ev.preventDefault();
								ev.stopPropagation();
				...
				var sClose = sCurDesc
					? sCurDesc[getElementsByClassName](s4)[0] || ""
					: "";

				var doOnClose = function doOnClose() {
					var timer = setTimeout(function() {
						clearTimeout(timer);
						timer = null;
				...
				if (sClose) {
					var handleSuperboxClose = function handleSuperboxClose(ev) {
						ev.preventDefault();
						ev.stopPropagation();

						sClose[_removeEventListener](
							"click",
							handleSuperboxClose
						);
			...
			var addItemHandler = function addItemHandler(e) {
				var handleSuperboxListItem = function handleSuperboxListItem(
					ev
				) {
					ev.stopPropagation();
					ev.preventDefault();
		...
		var addAppUpdatesLink = function addAppUpdatesLink() {
			var panel =
