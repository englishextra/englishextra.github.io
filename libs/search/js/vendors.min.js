...
(function(window, undefined) {
		...
		init: function init(selector, context, rootjQuery) {
			...
			if (typeof selector === "string") {
				...
				if (match && (match[1] || !context)) {
					...
					if (match[1]) {
						context =
							context instanceof jQuery ? context[0] : context; // scripts is true for back-compat
	...
	(function(window, undefined) {
	  	...
	  	---------------------------------------------------------------------- */
			...
			if (support.getById) {
				Expr.find["ID"] = function (id, context) {
					if (
						_typeof(context.getElementById) !== strundefined &&
						documentIsHTML
					) {
				...
				Expr.filter["ID"] = function (id) {
					...
					return function (elem) {
						var node =
							_typeof(elem.getAttributeNode) !== strundefined &&
			...
			Expr.find["TAG"] = support.getElementsByTagName
				? function(tag, context) {
						if (
							_typeof(context.getElementsByTagName) !==
							strundefined
						) {
			...
			Expr.find["CLASS"] =
				...
				function(className, context) {
					if (
						_typeof(context.getElementsByClassName) !==
							strundefined &&
						documentIsHTML
					) {
	  	...
	  	---------------------------------------------------------------------- */
			...
			if ((support.qsa = rnative.test(doc.querySelectorAll))) {
				...
				assert(function(div) {
					...
					div.innerHTML =
						"<select><option selected=''></option></select>"; // Support: IE8
									...
									if (forward && useCache) {
										...
										while (
											(node =
												(++nodeIndex &&
													node &&
													node[dir]) || // Fallback to seeking `elem` from the start
												(diff = nodeIndex = 0) ||
												start.pop())
										) {
		...
		support.sortStable =
			expando
				.split("")
				.sort(sortOrder)
				.join("") === expando; // Support: Chrome<14
	...
	jQuery.support = (function(support) {
		...
		div.innerHTML =
			"  <link/><table></table><a href='/a'>a</a><input type='checkbox'/>"; // Finish early in limited (non-browser) environments
		...
		support.checkClone = fragment
			.cloneNode(true)
			.cloneNode(true).lastChild.checked; // Support: IE<9
		...
		jQuery(function () {
			...
			support.reliableHiddenOffsets =
				isSupported && tds[0].offsetHeight === 0; // Check box-sizing and margin behavior.

			div.innerHTML = "";
			div.style.cssText =
				"box-sizing:border-box;-moz-box-sizing:border-box;-webkit-box-sizing:border-box;padding:1px;border:1px;display:block;width:4px;margin-top:1%;position:absolute;top:1%;"; // Workaround failing boxSizing test due to offsetWidth returning wrong value
	...
	jQuery.extend({
		...
		acceptData: function acceptData(elem) {
			...
			var noData =
				elem.nodeName && jQuery.noData[elem.nodeName.toLowerCase()]; // nodes accept data unless otherwise specified; rejection can be conditional
	...
	if (!getSetAttribute) {
		...
		jQuery.expr.attrHandle.id = jQuery.expr.attrHandle.name = jQuery.expr.attrHandle.coords = function ( // Some attributes are constructed with empty-string values when not defined
			elem,
			name,
			isXML
		) {
	...
	jQuery.event = {
		...
		add: function add(elem, types, handler, data, selector) {
			...
			if (!(eventHandle = elemData.handle)) {
				eventHandle = elemData.handle = function (e) {
					...
					return _typeof(jQuery) !== core_strundefined &&
		...
		trigger: function trigger(event, data, elem, onlyHandlers) {
			...
			event = event[jQuery.expando]
				? event
				: new jQuery.Event(type, _typeof(event) === "object" && event); // Trigger bitmask: & 1 for native handlers; & 2 for jQuery (always true)
	...
	function getAll(context, tag) {
		var elems,
			...
			i = 0,
			found =
				_typeof(context.getElementsByTagName) !== core_strundefined
					? context.getElementsByTagName(tag || "*")
					: _typeof(context.querySelectorAll) !== core_strundefined
			...
			for (; i < l; i++) {
				...
				if (elem || elem === 0) {
					...
					if (jQuery.type(elem) === "object") {
						...
						tmp =
							tmp ||
							safe.appendChild(context.createElement("div")); // Deserialize a standard representation
						...
						tmp.innerHTML =
							...
							wrap[2]; // Descend through wrappers to the right content
	...
	jQuery.extend({
		...
		style: function style(elem, name, value, extra) {
			...
			name =
				...
				(jQuery.cssProps[origName] = vendorPropName(style, origName)); // gets hook for the prefixed version
			...
			if (value !== undefined) {
				...
				if (type === "string" && (ret = rrelNum.exec(value))) {
					value =
						(ret[1] + 1) * ret[2] +
						parseFloat(jQuery.css(elem, name)); // Fixes bug #9237
	...
	function getWidthOrHeight(elem, name, extra) {
		...
		if (val <= 0 || val == null) {
			...
			valueIsBorderBox =
				isBorderBox &&
				(jQuery.support.boxSizingReliable || val === elem.style[name]); // Normalize "", auto, and prepare for extra
	  ...
	  */
		...
		ajax: function ajax(url, options) {
			...
			s.dataTypes = jQuery
				.trim(s.dataType || "*")
				.toLowerCase()
				.match(core_rnotwhite) || [""]; // A cross-domain request is in order when we have a protocol:host:port mismatch
			...
			if (!s.hasContent) {
				...
				if (s.data) {
					cacheURL = s.url +=
						(ajax_rquery.test(cacheURL) ? "&" : "?") + s.data; // #9682: remove data so that it's not used in an eventual retry
	...
	$.extend({
		...
		pnotify: function pnotify(options) {
			...
			var nonblock_pass = function nonblock_pass(e, e_name) {
				...
				var element_below = document.elementFromPoint(
					e.clientX,
					e.clientY
				);
				...
				pnotify.css(
					"cursor",
					cursor_style !== "auto" ? cursor_style : "default"
				);
			...
			if (_typeof(opts.styling) === "object") {
				styles = opts.styling;
			} else {
				...
				mouseenter: function mouseenter(e) {
					...
					if (opts.mouse_reset && animating === "out") {
						...
						pnotify.css("height", "auto").animate(
							{
								width: opts.width,
								opacity: opts.nonblock
									? opts.nonblock_opacity
									: opts.opacity
							},
							"fast"
						);
			...
			pnotify.pnotify = function (options) {
				...
				if (
					opts.icon !== old_opts.icon ||
					(opts.icon === true && opts.type !== old_opts.type)
				) {
			...
			pnotify.pnotify_position = function (dont_skip_hidden) {
				...
				if (!hidden || dont_skip_hidden) {
					...
					curpos1 = parseInt(
						pnotify.css(csspos1).replace(/(?:\..*|[^0-9.])/g, "")
					);
					...
					curpos2 = parseInt(
						pnotify.css(csspos2).replace(/(?:\..*|[^0-9.])/g, "")
					);
					...
					if (
						(s.dir1 === "down" &&
							s.nextpos1 + pnotify.height() >
								(s.context.is(body)
									? jwindow.height()
									: s.context.prop("scrollHeight"))) ||
						(s.dir1 === "up" &&
							s.nextpos1 + pnotify.height() >
								(s.context.is(body)
									? jwindow.height()
									: s.context.prop("scrollHeight"))) ||
						(s.dir1 === "left" &&
							s.nextpos1 + pnotify.width() >
								(s.context.is(body)
									? jwindow.width()
									: s.context.prop("scrollWidth"))) ||
						(s.dir1 === "right" &&
							s.nextpos1 + pnotify.width() >
								(s.context.is(body)
									? jwindow.width()
			...
			pnotify.pnotify_display = function () {
				...
				if (notices_data && notices_data.length > opts.maxonscreen) {
					...
					if (opts.stack.push !== "top") {
						el = notices_data.slice(
							0,
							notices_data.length - opts.maxonscreen
						);
					} else {
						el = notices_data.slice(
							opts.maxonscreen,
				...
				if (!pnotify.parent().length) {
					pnotify.appendTo(
						opts.stack.context ? opts.stack.context : body
					);
				}
			...
			pnotify.animate_in = function (callback) {
				...
				if (animation === "none") {
					...
					pnotify
						.show()
						.fadeTo(opts.animate_speed, opts.opacity, callback);
					...
					pnotify.show(
						animation,
						_typeof(opts.animation.options_in) === "object"
							? opts.animation.options_in
							: {},
						opts.animate_speed,
						callback
					);
			...
			pnotify.animate_out = function (callback) {
				...
				if (animation === "none") {
					...
					pnotify.hide(
						animation,
						_typeof(opts.animation.options_out) === "object"
							? opts.animation.options_out
							: {},
						opts.animate_speed,
						callback
					);
			...
			pnotify.pnotify_queue_remove = function () {
				pnotify.pnotify_cancel_remove();
				pnotify.timer = window.setTimeout(
					function() {
			...
			pnotify.sticker = $("<div />", {
				...
				.bind("pnotify_icon", function () {
					$(this)
						.children()
						.removeClass(styles.pin_up + " " + styles.pin_down)
						.addClass(opts.hide ? styles.pin_up : styles.pin_down);
				})
				.append(
					$("<span />", {
						class: styles.pin_up,
			...
			if (
				notices_data === null ||
				(typeof notices_data === "undefined"
					? "undefined"
					: _typeof(notices_data)) !== "object"
			) {
				notices_data = [];
			}
				...
				if (typeof history_menu === "undefined") {
					$("body")
						...
						.on("pnotify.history-last", function () {
							var pushTop =
								$.pnotify.defaults.stack.push === "top";
							var i = pushTop ? 0 : -1;
							var notice;
					...
					history_menu = $("<div />", {
						...
						.append(
							$("<button />", {
								class:
									"ui-pnotify-history-all " + styles.hi_btn,
						...
						.append(
							$("<button />", {
								class:
									"ui-pnotify-history-last " + styles.hi_btn,
		...
		if (document.createEvent && this.dispatchEvent) {
			...
			if (e.match(re_mouse_events)) {
				...
				event_object.initMouseEvent(
					...
					orig_e.metaKey,
					orig_e.button,
					orig_e.relatedTarget
				);
			} else if (e.match(re_ui_events)) {
				event_object = document.createEvent("UIEvents");
				event_object.initUIEvent(
					e,
					orig_e.bubbles,
					orig_e.cancelable,
					orig_e.view,
...
(function($, undefined) {
	$.widget("ui.menu", {
		...
		_create: function _create() {
			...
			this._on({
				"mousedown .ui-menu-item > a": function mousedownUiMenuItemA(
					event
				) {
					event.preventDefault();
				},
				"click .ui-state-disabled > a": function clickUiStateDisabledA(
					event
				) {
					event.preventDefault();
				},
				"click .ui-menu-item:has(a)": function clickUiMenuItemHasA(
					event
				) {
				...
				"mouseenter .ui-menu-item": function mouseenterUiMenuItem(
					event
				) {
