...
(function(root, document) {
	...
	var run = function run() {
		...
		var hideUiBtnsInFullScreen = function hideUiBtnsInFullScreen() {
			...
			var btnUiTotop =
				document[getElementsByClassName]("ui-totop")[0] || "";
			var holderSearchForm =
				document[getElementsByClassName]("holder-search-form")[0] || "";
			...
			if (!f) {
				...
				if (
					...
					root.fullScreen ||
					(root.innerWidth === screen.width &&
						root.innerHeight === screen.height)
				) {
		...
		var resizeHideUiBtnsInFullScreen = function resizeHideUiBtnsInFullScreen() {
			if (
				"undefined" !== typeof earlyDeviceType &&
				"desktop" === earlyDeviceType
			) {
		...
		var initDoSlide = function initDoSlide() {
			var getElementsByClassName = "getElementsByClassName";
			var _addEventListener = "addEventListener";
			var dsContainerSelector = ".ds-container";
			var dsContainer =
				document[getElementsByClassName]("ds-container")[0] || "";
			var cdPrev = document[getElementsByClassName]("cd-prev")[0] || "";
			var cdNext = document[getElementsByClassName]("cd-next")[0] || "";
			...
			var initScript = function initScript() {
				...
				var timer = function timer(slide, interval, token) {
				...
				var handleCdPrev = function handleCdPrev(ev) {
					ev.preventDefault();
					ev.stopPropagation();
					slide.prev();
				};

				var handleCdNext = function handleCdNext(ev) {
					ev.preventDefault();
					ev.stopPropagation();
					slide.next();
				};
		...
		var showMenuMore = function showMenuMore() {
			var holderPanelMenuMore =
				document[getElementsByClassName]("holder-panel-menu-more")[0] ||
				"";
			var isActiveClass = "is-active";
