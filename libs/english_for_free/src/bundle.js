/*jslint browser: true */
/*jslint node: true */
/*global doesFontExist, imagePromise, loadJsCss, addListener, removeListener, getByClass, addClass, hasClass,
removeClass, toggleClass, Promise, QRCode, require, ToProgress, unescape,
verge, VK, Ya*/
...
(function (root, document) {
	...
	var run = function () {
		...
		var initSuperBox = function () {
			...
			var handleItem = function (_this) {
				...
				if (link) {
					...
					var trackClicks = function (e) {
						...
						if ("undefined" !== typeof getHTTP && getHTTP()) {
							e.target = "_blank";
							e.rel = "noopener";
							addListener(e, "click", createCounterImg);
						} else {
							addListener(e, "click", handleSuperboxExternalLink);
						}
					};
					var j,
					m;
				...
				if (sClose) {
					var handleSuperboxClose = function (ev) {
						ev.preventDefault();
						ev.stopPropagation();
						removeListener(sClose, "click", handleSuperboxClose);
						doOnClose();
					};
					addListener(sClose, "click", handleSuperboxClose);
				}
			};
			var addItemHandler = function (e) {
				...
				addListener(e, "click", handleSuperboxListItem);
			};
			if (lists) {
				var i,
				l;
				for (i = 0, l = lists[_length]; i < l; i += 1) {
	...
	var load;
	load = new loadJsCss(["../../libs/english_for_free/css/bundle.min.css"], onFontsLoaded);
})("undefined" !== typeof window ? window : this, document);
