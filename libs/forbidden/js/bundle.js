...
(function(root, document) {
	...
	var setListener = function setListener(standard, fallback) {
	...
	root.ToProgress = (function() {
		var TP = function TP() {
			...
			ToProgress.prototype.finish = function(callback) {
				...
				if (transitionEvent) {
					this.progressBar.addEventListener(transitionEvent, function(
						e
					) {
						that.reset();
	...
	root.manageExternalLinkAll = function() {
		...
		var arrange = function arrange(e) {
			var handleLink = function handleLink(url, ev) {
				ev.stopPropagation();
				ev.preventDefault();
	...
	root.earlyDeviceFormfactor = (function(selectors) {
		...
		var addClasses = function addClasses(e) {
		...
		var removeClasses = function removeClasses(e) {
			var classesList = e.split(" ");
		...
		var toggleOrientationClasses = function toggleOrientationClasses(
			mqList,
			classText
		) {
			var handleMq = function handleMq(mqList) {
				if (mqList[matches]) {
		...
		var toggleSizeClasses = function toggleSizeClasses(mqList, classText) {
			var handleMq = function handleMq(mqList) {
				if (mqList[matches]) {
					addClasses(classText);
					size = classText;
		...
		for (key in orientationMq) {
			if (orientationMq.hasOwnProperty(key)) {
				toggleOrientationClasses(
					root[matchMedia](orientationMq[key]),
					key
				);
			}
		}
	...
	root.earlySvgSupport = (function(selector) {
		selector = document.implementation.hasFeature(
			"http://www.w3.org/2000/svg",
			"1.1"
		)
			? selector
...
(function(root, document) {
	...
	var run = function run() {
		...
		var manageNavMenu = function manageNavMenu() {
			...
			var holderPanelMenuMore =
				getByClass(document, "holder-panel-menu-more")[0] || "";
			var locHref = root.location.href || "";
			...
			var removeHolderActiveClass = function removeHolderActiveClass() {
				if (
					holderPanelMenuMore &&
					hasClass(holderPanelMenuMore, isActiveClass)
				) {
			...
			if (page && container && btn && panel && panelItems) {
				addContainerHandler();
				addBtnHandler();
				addItemHandlerAll();
			}
		...
		var addUpdateAppLink = function addUpdateAppLink() {
			...
			if (/Windows/i.test(navUA) && /(WOW64|Win64)/i.test(navUA)) {
				...
				/(x86_64|x86-64|x64;|amd64|AMD64|x64_64)/i.test(navUA) &&
				/(Linux|X11)/i.test(navUA)
			) {
				linkHref =
			...
			var arrange = function arrange() {
				...
				link.appendChild(
					document.createTextNode("Скачать приложение сайта")
				);
		...
		var manageMenuMore = function manageMenuMore() {
			var container = document.getElementById("container") || "";
			var page = document.getElementById("page") || "";
			var holder =
				getByClass(document, "holder-panel-menu-more")[0] || "";
			...
			if (page && container && holder && btn && panel && panelItems) {
				addContainerHandler();
				addBtnHandler();
				addItemHandlerAll();
			}
		...
		var manageYaShare2Btn = function manageYaShare2Btn() {
				...
				var logic = function logic() {
					...
					var initScript = function initScript() {
						try {
							...
							throw new Error(
								"cannot root.yaShare2Instance.updateContent or Ya.share2 " +
									err
							);
						}
					};
		...
		var manageVkLikeBtn = function manageVkLikeBtn() {
	...
	loadDeferred(
		["/libs/forbidden/css/bundle.min.css"],
