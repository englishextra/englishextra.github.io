...
removeClass, renderTemplate, safelyParseJson, scroll2Top, setDisplayNone,
setVisible, supportsCanvas, supportsPassive, supportsSvgSmilAnimation, t,
throttle, toggleClass, ToProgress, VK, WheelIndicator, Ya, Zoomwall*/
...
(function (root, document) {
			...
			var generate = function (responseText) {
				...
				return new Promise(function (resolve, reject) {
					...
					try {
						...
						if (!jsonObject.pages[0][jsonSrcKeyName]) {
							...
							if (!jsonObject.pages[0][jsonTitleKeyName]) {
					...
					insertFromTemplate(jsonObject, "template_zoomwall", "render_zoomwall", function () {
			...
			if (root.Zoomwall && zoomwall) {
				ajaxGet(jsonUrl, {
					...
					failure: function (status) {
						throw new Error("Cannot create Zoomwall " + status);
					}
				});
			}
		};
	...
	var urlArray = ["./libs/picturewall/css/bundle.min.css"];
