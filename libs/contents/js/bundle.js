...
(function(root, document) {
	...
	var run = function run() {
		...
		var getKeyValuesFromJSON = function getKeyValuesFromJSON(b, d) {
			var c = [];

			for (var a in b) {
				if (b.hasOwnProperty(a)) {
					if ("object" === _typeof(b[a])) {
		...
		var initMasonryDisqus = function initMasonryDisqus() {
			...
			var gridItem =
				document[getElementsByClassName](gridItemClass) || "";
					...
					if (root.Packery) {
						...
						if (gridItem) {
							if (root.Draggabilly) {
								var draggie,
									draggies = [],
									initDraggie = function initDraggie(e) {
										var draggableElem = e;
			...
			var showDisqusThread = function showDisqusThread() {
				var initDisqus = function initDisqus() {
					var timer = setInterval(function() {
						var disqusThreadHeight =
							disqusThread.clientHeight ||
							disqusThread.offsetHeight ||
							"";
			...
			var initScript = function initScript() {
				...
				if (disqusThread && disqusThreadShortname) {
					if ("undefined" !== typeof getHTTP && getHTTP()) {
						...
						setStyleDisplayNone(
							disqusThread[parentNode][parentNode]
						);
					}
				}
			};
		...
		var handleContentsSelect = function handleContentsSelect() {
			var _this = this;
			...
			if (hashString) {
				var targetObj = isValidId(hashString, true)
					? document[getElementById](hashString.replace(/^#/, "")) ||
					  ""
					: "";
		...
		var manageContentsSelect = function manageContentsSelect() {
			var contentsSelect =
			...
			var processJsonResponse = function processJsonResponse(
				...
				var generateContentsOptions = function generateContentsOptions(
					e
				) {
					...
					if (label && link) {
						...
						contentsOption[appendChild](
							document[createTextNode](
								truncString("" + label, 33)
							)
						);
				...
				contentsSelect[_addEventListener](
					"change",
					handleContentsSelect
				);
			};
	...
	loadCSS(
		...
		"../libs/contents/css/bundle.min.css",
