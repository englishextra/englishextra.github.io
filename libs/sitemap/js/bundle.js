...
(function(root, document) {
	...
	var run = function run() {
		...
		var initMasonry = function initMasonry() {
			...
			var holder =
				document[getElementsByClassName]("holder-filter-buttons")[0] ||
				"";
			var btn = holder ? holder[getElementsByTagName]("li") || "" : "";
			var sel =
				document[getElementsByClassName]("filter-select")[0] || "";
			var controls =
				document[getElementsByClassName]("holder-filter-controls")[0] ||
				"";
			var iso;
			var msnry;
			var pckry;
			...
			var initScript = function initScript() {
				...
				if (root.Masonry && root.Isotope) {
					...
					var handleFilterButtonAll = function handleFilterButtonAll() {
						var _this = this;
					...
					var handleFilterSelect = function handleFilterSelect() {
						var _this = this;
						...
						for (var j = 0, m = btn.length; j < m; j += 1) {
							if (
								btn[j].dataset.filter ===
								_this.options[_this.selectedIndex].value
							) {
	...
	/* var scripts = ["./libs/sitemap/css/bundle.min.css"]; */
	...
	loadCSS(
		...
		"./libs/sitemap/css/bundle.min.css",
