...
(function(root, document) {
	...
	var run = function run() {
					...
					$(document).ready(function() {
						...
						if (commentsTextarea) {
							var handleSubmitBtn = function handleSubmitBtn() {
								...
								var locationHref = encodeURIComponent(
									root.location.href.split("#")[0]
								);
								var docTitle = encodeURIComponent(
								...
								if (commentsTextareaValue) {
									localStorage.setItem(
										commentsTextareaId,
										JSON.stringify(commentsTextareaValue)
									);
								}
								...
								$.ajax({
									...
									success: function success() {
										...
										loadComments.load(
											"/scripts/comments/?load=posts&limit=100"
										);
										notify(successMsg);
									}
								});
							...
							var commentsTextareaValue = commentsTextarea.val();
							var commentsTextareaStoredValue = localStorage.getItem(
								commentsTextareaId
							);

							if (
								!commentsTextareaValue &&
								commentsTextareaStoredValue
							) {
								commentsTextarea.val(
									$.parseJSON(commentsTextareaStoredValue)
								);
							}
      ...
      if (!scriptIsLoaded(jsUrl)) {
		...
		var manageCommentsTextarea = function manageCommentsTextarea() {
